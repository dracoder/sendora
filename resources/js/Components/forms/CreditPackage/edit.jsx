...
export default function EditForm({ onSubmit, open, onOpenChange, editingId }) {
    ...
    useEffect(() => {
        if (!editingId) return;
        CreditPackageService.show(editingId, (data) => {
            reset(data);
        });
    }, [editingId]);

    const handleUpdateAction = (data) => {
        CreditPackageService.update(editingId, data, () => {
            toast({
                title: t('pages.updated', { name: t('pages.credit_packages') }),
                duration: 3000
            });
            reset();
            onOpenChange(false);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{t('pages.edit', { name: t('pages.credit_packages') })} #{editingId}</DialogTitle>
                </DialogHeader>
