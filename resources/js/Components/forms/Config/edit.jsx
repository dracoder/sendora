...
import configFormSchema from '@/Validation/configFormSchema';
...
export default function EditForm({ onSubmit, open, onOpenChange, editingId }) {
    ...
    useEffect(() => {
        if (!editingId) return;
        ConfigService.show(editingId, (data) => {
            reset(data);
        });
    }, [editingId]);

    const handleUpdateAction = (data) => {
        ConfigService.update(editingId, data, () => {
            toast({
                title: t('pages.updated', { name: t('pages.config') }),
                duration: 3000
            });
            reset();
            onOpenChange(false);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{t('pages.edit', { name: t('pages.config') })} - {watch('key')}</DialogTitle>
                </DialogHeader>
