...
import apiKeyFormSchema from '@/Validation/apiKeyFormSchema';
...
export default function EditForm({ onSubmit, open, onOpenChange, editingId }) {
    ...
    useEffect(() => {
        if (!editingId) return;
        ApiKeyService.show(editingId, (data) => {
            reset(data);
        });
    }, [editingId]);

    const handleUpdateAction = (data) => {
        ApiKeyService.update(editingId, data, () => {
            toast({
                title: t('pages.updated', { name: t('pages.api-key') }),
                duration: 3000
            });
            reset();
            onOpenChange(false);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{t('pages.edit', { name: t('pages.api-key') })} #{editingId}</DialogTitle>
                </DialogHeader>
